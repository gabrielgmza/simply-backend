...
export const systemSettingsService = {
  ...
  // Obtener todas (devuelve array flat para el frontend)
  async getAll() {
    const settings = await prisma.system_settings.findMany({
      orderBy: [{ category: 'asc' }, { key: 'asc' }]
    });

    // Devolver array flat con todos los campos necesarios
    return settings.map(s => ({
      ...
      data_type: s.value_type,
      is_sensitive: s.is_sensitive,
      updated_at: s.updated_at
    }));
  },

  // Obtener todas agrupadas por categoría
  async getAllGrouped() {
    const settings = await prisma.system_settings.findMany({
      orderBy: [{ category: 'asc' }, { key: 'asc' }]
    });
