...
export const rbacService = {
  ...
  async assignPermissionToRole(
    ...
    const result = await prisma.role_permissions.create({
      data: {
        ...
        conditions: options?.conditions ? JSON.parse(JSON.stringify(options.conditions)) : Prisma.DbNull,
    ...
    if (options?.grantedBy) {
      await auditLogService.log({
        ...
        description: `Permission ${permissionId} granted to role ${roleId}`,
        newData: {
          roleId,
          permissionId,
  ...
  async initializeDefaultPermissions() {
    const resources = [
      ...
      'dashboard', 'aria', 'reports', 'providers', 'rewards'
    ];
