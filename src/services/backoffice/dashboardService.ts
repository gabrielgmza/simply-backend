...
export const dashboardService = {
  ...
  async getRecentActivity(limit: number = 10) {
    ...
    const activity = [
      ...transactions.map(t => ({
        ...
        status: t.status,
        user: t.users ? `${t.users.first_name} ${t.users.last_name}` : 'N/A',
      ...
      ...investments.map(i => ({
        ...
        status: i.status,
        user: i.users ? `${i.users.first_name} ${i.users.last_name}` : 'N/A',
      ...
      ...financings.map(f => ({
        ...
        status: f.status,
        user: f.users ? `${f.users.first_name} ${f.users.last_name}` : 'N/A',
  ...
  async getTopPerformers() {
    ...
    const users = await prisma.users.findMany({
      where: { id: { in: userIds } },
      select: { id: true, first_name: true, last_name: true, email: true, level: true }
    });
