...
export const stripeService = {
  ...
  async handlePIXWebhook(event: Stripe.Event): Promise<void> {
    ...
    if (newStatus === 'completed') {
      ...
      await prisma.transactions.create({
        data: { user_id: dbPayment.user_id, type: 'DEPOSIT', amount: new Prisma.Decimal(amountARS), currency: 'ARS', status: 'COMPLETED', description: `Depósito PIX - R$ ${dbPayment.amount}`, reference: intent.id, metadata: { originalAmount: Number(dbPayment.amount), originalCurrency: 'BRL' } }
      });
    }
  },
