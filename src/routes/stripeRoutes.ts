...
router.get('/bcra/usuario/:userId', authMiddleware, requirePermission('users:read'), async (req: AuthRequest, res) => {
  try {
    ...
    const user = await prisma.users.findUnique({
      ...
      select: { id: true, dni: true, first_name: true, last_name: true }
    });

    if (!user || !user.dni) {
      return res.status(400).json({ success: false, error: 'Usuario sin DNI/CUIT registrado' });
    }

    // DNI a CUIT (simplificado - en producción usar validación real)
    const cuit = `20${user.dni}9`;
    
    const evaluacion = await bcraService.evaluarRiesgoCrediticio(cuit);
    res.json({ 
      success: true, 
      data: { 
        usuario: { id: user.id, nombre: `${user.first_name} ${user.last_name}`, cuit },
        ...evaluacion 
      } 
    });
  } catch (error: any) {
